...
def interpolate(age, mass, iso_grid, age_arr, filters, zero_points):
    ...
    s1 = isoInterp(mass, a1, iso_grid, filters, zero_points)
    s2 = isoInterp(mass, a2, iso_grid, filters, zero_points)

    # Interpolate along age between s1 and s2
    if s1 is not None and s2 is not None:
        s = ageInterp(age, s1, a1, s2, a2, age_arr, zero_points)
        return s
    return None
...
def isoInterp(mass, age_idx, iso_grid, filters, zero_points):
    ...
    s1_filt1_flux = mag_to_flux(s1_filt1, zero_points[0])
    s1_filt2_flux = mag_to_flux(s1_filt2, zero_points[1])
    ...
    s2_filt1_flux = mag_to_flux(s2_filt1, zero_points[0])
    s2_filt2_flux = mag_to_flux(s2_filt2, zero_points[1])
    ...
    s_filt1_flux = w1 * s1_filt1_flux + w2 * s2_filt1_flux
    s_filt2_flux = w1 * s1_filt2_flux + w2 * s2_filt2_flux

    # Convert interpolated fluxes back to magnitudes
    s_filt1 = flux_to_mag(s_filt1_flux, zero_points[0])
    s_filt2 = flux_to_mag(s_filt2_flux, zero_points[1])
...
def mag_to_flux(magnitude, zero_point_flux):
    ans = np.power(10, (magnitude - 0.03) / -2.5) * zero_point_flux
    return ans

def flux_to_mag(flux, zero_point_flux):
    return -2.5 * np.log10(flux / zero_point_flux) + 0.03
...
def ageInterp(age, s1, a1, s2, a2, age_arr, zero_points):
    ...
    # convert intermediate magnitudes to fluxes
    flux11 = mag_to_flux(s1[4], zero_points[0])
    flux12 = mag_to_flux(s1[5], zero_points[1])
    flux21 = mag_to_flux(s2[4], zero_points[0])
    flux22 = mag_to_flux(s2[5], zero_points[1])
    ...
    s_filt1_flux = w1 * flux11 + w2 * flux21
    s_filt2_flux = w1 * flux12 + w2 * flux22
