...
async:
    nohup python3 run-fits.py \
    --dat /scratch/wyz5rge/synthetic-hr/data/phot_joined_avmass.dat \
    --out_root results_stepwise \
    --base_iso_dir isochrones_stepwise \
    --dist 4500 \
    --metallicity 0.0 \
    --log_age 6.0 \
    --interp_n 10 \
    --av_grid_halfwidth 5.0 \
    --av_grid_step 0.1 \
    --av_max 70 \
    --conf 0.95 \
    --nbins_logM 25 \
...
# -----------------------------
def plot_cmd_and_av_mass_acceptance(
    arr: np.ndarray,
    ...
    obs_e162: Optional[float] = None,
    obs_m182: Optional[float] = None,
    obs_e182: Optional[float] = None,
) -> None:
    """
    Left panel:
      - observed CMD point with error bars
    ...
    def get_interp_for_av(av_value: float):
        ...
        aks = av_value * fitter.AKs_per_AV
        lock = fitter._acquire_lock()
        try:
            iso = synthetic.IsochronePhot(
                fitter.log_age,
                AKs=aks,
                distance=fitter.dist,
                metallicity=fitter.metallicity,
                evo_model=fitter.evo_model,
                atm_func=fitter.atm_func,
                red_law=fitter.red_law,
                filters=fitter.SUPER_FILT_LIST,
                iso_dir=fitter.iso_dir,
            )
        finally:
            fitter._release_lock(lock)
        return fitter.interpolate_isochrone_superset(iso, num_interp=fitter.interp_n)

    # isochrones for CMD panel
    iso0 = get_interp_for_av(0.0)
    iso_best = get_interp_for_av(float(best["AV"]))
    iso_lo = get_interp_for_av(max(0.0, float(best["AV"]) - 1.0))
    iso_hi = get_interp_for_av(min(fitter.av_max, float(best["AV"]) + 1.0))
    ...
    obs_mag = None
    obs_mag_err = None
    if None not in (obs_m162, obs_e162, obs_m182, obs_e182):
        obs_color = float(obs_m162 - obs_m182)
        obs_color_err = float(math.sqrt(obs_e162**2 + obs_e182**2))
        obs_mag = float(obs_m162)
        obs_mag_err = float(obs_e162)

    # right-panel points
    if acceptable is not None and acceptable.size > 0:
        show = acceptable
        subtitle = "acceptance region shown"
    else:
        k = min(300, a.size)
    ...
    ax.scatter(
        ...
        label="Best-fit grid point", zorder=6
    )
    ...
    if obs_color is not None:
        ax.errorbar(
            [obs_color], [obs_mag],
            xerr=[obs_color_err], yerr=[obs_mag_err],
    ...
    # ---------------- right: AV-mass ----------------
    ax = axes[1]
    sc2 = ax.scatter(show["mass"], show["AV"], c=show["chi2"], s=10, alpha=0.75)
    fig.colorbar(sc2, ax=ax, label=r"$\chi^2$")
    ...
    if true_av is not None and true_mass is not None:
        if np.isfinite(true_av) and np.isfinite(true_mass) and true_av >= 0 and true_mass > 0:
            ax.scatter(
                [true_mass], [true_av],
                marker="X", s=140, color="limegreen",
                edgecolor="k", linewidth=0.8,
                label="Andersen truth", zorder=6
            )

    ax.set_xscale("log")
    ax.set_xlabel("Mass (M$_\\odot$)")
    ax.set_ylabel("AV")
    ax.set_title("AV-mass acceptance view")
...
class SupersetGridFitter:
    ...
    def __init__(
        ...
        self.atm_func = atmospheres.get_BTSettl_2015_atmosphere
    ...
    def _cmd_obs(m162: float, m182: float) -> Tuple[float, float]:
        # Andersen-style A1 CMD: color = F162M - F182M, magnitude = F162M
        return (m162 - m182, m162)

    @staticmethod
    def _cmd_sigma(e162: float, e182: float) -> Tuple[float, float]:
        # sigma(color)^2 = e162^2 + e182^2 ; sigma(magnitude)=e162
        return (math.sqrt(e162 * e162 + e182 * e182), e162)
    ...
    def minimize_AKs_from_162_182(self, m162: float, m182: float) -> float:
        ...
        aks_max = self.av_max * self.AKs_per_AV
        res = minimize_scalar(objective, bounds=(0.0, aks_max), method="bounded")
        return float(res.x)
    ...
    def analyze_star_for_runs(
        ...
        av_hi = min(self.av_max, av_guess + self.av_grid_halfwidth)
        ...
        results_by_run: Dict[str, List[Tuple[float, float, float, float, float, float]]] = {
            cfg.name: [] for cfg in runs_needed
        }
        ...
        for av in av_grid:
            ...
            for entry in interp:
                ...
                for cfg in runs_needed:
                    ...
                    results_by_run[cfg.name].append((
                        float(av),
                        float(aks),
                        mass_val,
                        float(chi2_val),
                        float(entry["m_jwst_F162M"]),
                        float(entry["m_jwst_F182M"]),
                    ))
        ...
        for cfg in runs_needed:
            ...
            plot_cmd_and_av_mass_acceptance(
                arr=arr,
                ...
                obs_e162=float(row["e162"]),
                obs_m182=float(row["F182M"]),
                obs_e182=float(row["e182"]),
            )
...
def main():
    ...
    parser.add_argument("--av_max", type=float, default=70.0)
    ...
    runs: List[FitRunConfig] = [
        FitRunConfig(
            name="jwst_cmd_F162MminusF182M_vs_F162M",
