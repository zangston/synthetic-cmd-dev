...
# ## Static 1 Myr CMD color bins with a global 3000--3600 K temperature cut
#
...
# epsilon_ff=infinity matched control.
#
# This version implements the fixed-bin definition discussed for the
# multi-CMD spread analysis.
#
# CMD BIN DEFINITION
# ------------------
# For EACH CMD, use the 1 Myr fiducial isochrone only:
#
#   1. determine the CMD color at Teff = 3000 K;
#   2. determine the CMD color at Teff = 3600 K;
#   3. take the interval between those two colors;
#   4. divide that interval into six exactly equal-width COLOR bins;
#   5. freeze those seven color edges and use them at every cluster age.
#
# The color at 3000 K and 3600 K is obtained by local interpolation of color
# versus Teff on the 1 Myr isochrone near the target low-temperature branch.
#
# TEMPERATURE FILTER
# ------------------
# At every snapshot, a catalog star is eligible for the spread analysis only
# if
#
#       3000 K <= Teff <= 3600 K.
#
# This is a GLOBAL eligibility cut only. It does NOT assign stars to 100 K
# temperature sub-bins. Once a star passes the temperature cut, its ACTUAL
# CMD color decides which of the six fixed color bins it enters.
#
# HR DIAGRAM
# ----------
# The HR diagram follows the original Masegian-style construction directly:
# six fixed 100 K Teff bins from 3000 to 3600 K.
#
# PLOTTING
# --------
# Isochrone points are rendered with scatter(), not by connecting a selected
# subset with ax.plot(). This avoids artificial solid lines between disjoint
# parts of an isochrone.
#
# Existing expensive interpolation caches are reused. A NEW metric cache is
# used because the bin definition differs from earlier runs.
...
OUTPUT_DIR = Path.cwd() / 'seed00_static_1myr_color_bins_teff_band_outputs'
...
# New metric cache: do not reuse either of the previous metric definitions.
METRIC_DIR = CACHE_DIR / 'metrics_static_1myr_color_bins_teff_band'
...
def cache_paths(
    ...
    return (
        directory
        / f'catalog_t{time_myr:04.1f}myr.csv',
        directory
        / f'accounting_t{time_myr:04.1f}myr.json',
    )
...
for time_myr in ANALYSIS_TIMES_MYR:
    ...
    for case, builder, recompute in [
        ...
        if (
            ...
            df, accounting = load_catalog(
                case,
                time_myr,
            )

            print(
                f'  loaded cached {case}'
            )
        ...
        accounting_rows.append({
            **accounting,
            'status': accounting.get(
                'status',
                'success',
            ),
            'error': accounting.get(
                'error',
                '',
            ),
        })
...
df_accounting.to_csv(
    OUTPUT_DIR
    / 'finite_and_infinite_interpolation_accounting.csv',
    index=False,
)
...
for diagram in DIAGRAMS:
    ...
    ax.set_title(
        ...
        + r'$3000\leq T_{\rm eff}\leq3600$ K, '
        + (
            'six equal-width Teff bins'
            if diagram.kind == 'hr'
            else 'six fixed 1 Myr-derived equal-width color bins'
        )
    )
    ...
    finish_figure(
        fig,
        f'spread_comparison_{diagram.key}_equal_bins.png',
    )
