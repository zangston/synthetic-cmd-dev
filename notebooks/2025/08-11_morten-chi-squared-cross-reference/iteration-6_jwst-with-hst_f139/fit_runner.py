...
parser = argparse.ArgumentParser(description="Run chi-squared fitting for JWST+HST F139M photometry.")
parser.add_argument("--dat", type=str, required=True)
args = parser.parse_args()

output_csv = "fit_results_jwst_hst_f139.csv"
fitter = ChiSquaredFitter(
    filt_list=['jwst,F162M','jwst,F182M','jwst,F200W','wfc3,ir,f139m'],
    filters=['m_jwst_F162M','m_jwst_F182M','m_jwst_F200W','m_hst_f139m'],
    output_dir='./fit_plots_jwst_hst_f139'
)
...
def parse_line(line):
    ...
    mags = [p[2], p[4], p[6], p[10]]
    errs = [p[3], p[5], p[7], p[11]]
    av, mass = p[14], p[15]
