...
class ChiSquaredFitter:
    ...
    def interpolate_isochrone(self, isochrone, num_interp=10):
        ...
        # interpolate across all chosen filters
        for i in range(len(masses) - 1):
            mass1, mass2 = masses[i], masses[i+1]
            interp_masses = np.linspace(mass1, mass2, num=num_interp+2)[1:-1]
            interp_row = {f: np.linspace(isochrone.points[f][i], isochrone.points[f][i+1], num=num_interp+2)[1:-1]
        ...
        # add the original grid points too
    ...
    def compute_squared_distance(self, dered_mags, interp_table):
        # Use first two filters for color, second filter for magnitude
    ...
    def analyze_line(self, index, mags, errs, true_av, true_mass):
        ...
        if not acceptable_results:
            ...
            return {
                'index': index, 'best_mass': best_fit['mass'], 'best_AV': best_fit['AV'],
                'mass_min': None, 'mass_max': None, 'AV_min': None, 'AV_max': None,
                'intersects': None, 'min_chi2': min_chi2
            }
        ...
        return {
            ...
            'intersects': intersects, 'min_chi2': min_chi2
        }
