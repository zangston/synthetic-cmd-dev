...
class ChiSquaredFitter:
    def __init__(self, filt_list, filters, base_iso_dir='isochrones', dist=4500,
                 metallicity=0, log_age=6.0, output_dir='fit_plots'):
        # Separate cache dirs so JWST-only and JWST+HST runs don’t collide
        if len(filt_list) == 3:
            self.iso_dir = os.path.join(base_iso_dir, "jwst_only")
        else:
            self.iso_dir = os.path.join(base_iso_dir, "jwst_hst")
        ...
        # Add all JWST + HST filters used in your photometry
    ...
    def interpolate_isochrone(self, isochrone, num_interp=10):
        ...
        # Interpolate for all chosen filters
    ...
    def chi_squared_grid(self, AV_grid, mags, errs):
        ...
        for av in AV_grid:
            ...
            for entry in interp:
                ...
                chi_sq = np.sum(((mags[i] - model_mags[i])**2) / max(errs[i]**2, 1e-3)
    ...
    def plot_results(self, index, grid_results, best_fit, true_av, true_mass, chi2_threshold):
        ...
        AV_idx = {av: i for i, av in enumerate(AV_vals)}
        mass_idx = {m: i for i, m in enumerate(mass_vals)}
        for r in grid_results:
            chi2_grid[mass_idx[r['mass']], AV_idx[r['AV']]] = r['chi2']

        AVs, masses = np.meshgrid(AV_vals, mass_vals)
        plt.figure(figsize=(10,7))
        mask = chi2_grid < chi2_threshold
        sc = plt.scatter(AVs[mask], masses[mask], c=chi2_grid[mask], cmap='seismic', s=10)
        plt.colorbar(sc, label='Reduced χ²')
        plt.scatter([best_fit['AV']], [best_fit['mass']], c='lime', marker='x', s=100, label='Best fit')
        plt.scatter([true_av], [true_mass], c='gold', marker='*', s=150, label='Ground truth')
        plt.xlabel("AV (Visual Extinction)")
        plt.ylabel("Mass (M☉)")
        plt.title(f"Acceptable Region – Line {index}")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, f"fit_{index}.png"))
        plt.close()